...
import {
  User, Calendar, CheckCircle2, XCircle, Plus, Phone, Trash2,
  ChevronRight, ChevronLeft, Target, AlertTriangle, Settings, Users, Palmtree, ArrowRight, Loader2, Mail, WifiOff, RefreshCw, LayoutGrid, BarChart3, PieChart, FileJson, Zap, X, Edit2
} from "lucide-react";
...
// --- Memoized Components to prevent 4-second freezing ---
const StudentRowDesktop = React.memo(({ student, daysInfo, mAbs, absenceThreshold, attendanceMap, onToggle, onSelect }: any) => {
  return (
    <tr className="border-b hover:bg-blue-50/10 transition-all group">
      <td className="sticky right-0 z-20 bg-white group-hover:bg-[#fcfdfe] p-5 text-right border-l font-bold shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)]">
        <button onClick={() => onSelect(student)} className="flex items-center gap-3 text-right w-full hover:text-blue-600 transition-all">
          <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center transition-all", student.gender === 'female' ? "bg-pink-50 text-pink-400" : "bg-blue-50 text-blue-400")}>
            <User size={20} />
          </div>
          <span className="text-sm font-black text-slate-800">{student.name}</span>
        </button>
      </td>
      <td className="p-5 text-center border-l font-black">
        <div className={cn("w-12 h-10 rounded-lg flex items-center justify-center text-xs mx-auto border", mAbs >= absenceThreshold ? "bg-red-600 text-white border-red-700 shadow-md" : "bg-white text-slate-600 border-slate-100")}>{mAbs}</div>
      </td>
      {daysInfo.map((d: any) => {
        const status = attendanceMap[`${student.id}-${d.dateStr}`];
        return (
          <td key={d.dateStr} className={cn("p-2 text-center border-l cursor-pointer select-none transition-all", d.isHol && "bg-slate-50/50 pointer-events-none")} onClick={() => onToggle(student.id, d.date)} onContextMenu={(e) => { e.preventDefault(); onToggle(student.id, d.date, 'excused'); }}>
            {!d.isHol ? (
              <div className={cn("w-7 h-7 rounded-lg flex items-center justify-center mx-auto transition-all", status === 'absent' ? "bg-red-600 text-white shadow-md scale-110" : status === 'excused' ? "bg-amber-500 text-white shadow-md scale-110" : status === 'present' ? "bg-green-500 text-white shadow-md scale-110" : "bg-white text-slate-200 border border-slate-100 shadow-sm")}>
                {status === 'absent' ? <XCircle size={14} strokeWidth={3} /> : status === 'excused' ? "📝" : status === 'present' ? <CheckCircle2 size={14} strokeWidth={3} /> : <div className="w-2 h-2 rounded-full border border-slate-100"></div>}
              </div>
            ) : <Palmtree size={12} className="mx-auto text-orange-200 opacity-40" />}
          </td>
        );
      })}
    </tr>
  ...
  if (prev.mAbs !== next.mAbs) return false;
  if (prev.daysInfo !== next.daysInfo) return false;
  for (let i = 0; i < next.daysInfo.length; i++) {
    const key = `${next.student.id}-${next.daysInfo[i].dateStr}`;
    if (prev.attendanceMap[key] !== next.attendanceMap[key]) return false;
  }
  return true;
});

const StudentCardMobile = React.memo(({ student, workingDaysInfo, monthAbs, totalAbs, attendanceMap, onToggle, onSelect, scrollToToday }: any) => {
  return (
    <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden animate-in fade-in">
      <div className="p-5 flex justify-between items-start">
        <div className="flex gap-4"><div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg", student.gender === 'female' ? "bg-pink-50 text-pink-500" : "bg-blue-50 text-blue-500")}>{student.name.charAt(0)}</div><div><h3 className="font-black text-slate-800 text-base">{student.name}</h3><p className="text-[10px] font-bold text-slate-400" dir="ltr">{student.parent_phone}</p></div></div>
        <div className="text-left"><p className="text-[8px] font-black text-slate-400 uppercase">غياب الشهر</p><p className="text-xl font-black text-red-600 leading-tight">{monthAbs} يوم</p></div>
      </div>
      <div className="px-5 pb-5">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar py-2">
          {workingDaysInfo.map((d: any) => {
            const status = attendanceMap[`${student.id}-${d.dateStr}`];
            return (
              <button key={d.dateStr} data-today={d.isToday} onClick={() => onToggle(student.id, d.date)} onContextMenu={(e) => { e.preventDefault(); onToggle(student.id, d.date, 'excused'); }}
                className={cn("flex-shrink-0 w-12 flex flex-col items-center gap-1.5 p-2 rounded-2xl border transition-all", d.isToday ? "bg-blue-600 border-blue-600 shadow-lg text-white" : "bg-slate-50 border-slate-100")}>
                <span className={cn("text-[8px] font-black", d.isToday ? "text-blue-100" : "text-slate-400")}>{d.dayNumMonth}</span>
                ...
                <span className={cn("text-[8px] font-black", d.isToday ? "text-white" : "text-slate-400")}>{d.dayName}</span>
              </button>
            );
          })}
        </div>
      </div>
      <div className="p-4 bg-slate-50 flex justify-between items-center text-[10px] font-black"><div className="flex items-center gap-1.5 text-green-600"><Target size={12} /><span>نسبة الالتزام: {Math.max(0, 100 - (totalAbs * 3))}%</span></div><button onClick={() => onSelect(student)} className="text-blue-600 flex items-center gap-1 font-bold"><span>التفاصيل</span><ChevronLeft size={12} /></button></div>
    </div>
  );
}, (prev: any, next: any) => {
  if (prev.student.id !== next.student.id) return false;
  if (prev.monthAbs !== next.monthAbs) return false;
  if (prev.workingDaysInfo !== next.workingDaysInfo) return false;
  for (let i = 0; i < next.workingDaysInfo.length; i++) {
    const key = `${next.student.id}-${next.workingDaysInfo[i].dateStr}`;
    if (prev.attendanceMap[key] !== next.attendanceMap[key]) return false;
  }
  return true;
});

const StudentRowMobileFast = React.memo(({ student, todayStrLocal, status, onToggle, onSelect }: any) => {
  return (
    <div className="p-4 flex items-center justify-between group active:bg-slate-50 transition-all select-none gap-4">
      <button onClick={() => onSelect(student)} className="flex items-center gap-3 cursor-pointer flex-1 text-right bg-transparent border-none outline-none">
        <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black flex-shrink-0", student.gender === 'female' ? "bg-pink-50 text-pink-500" : "bg-blue-50 text-blue-500")}>{student.name.charAt(0)}</div>
        <span className="font-black text-slate-800">{student.name}</span>
      </button>
      <button
        onClick={() => {
          console.log('[FAST MODE] Button Clicked for:', student.name);
          onToggle(student.id, new Date());
        }}
        onContextMenu={(e) => { e.preventDefault(); onToggle(student.id, new Date(), 'excused'); }}
        className={cn("w-14 h-14 rounded-2xl border-2 flex flex-shrink-0 items-center justify-center transition-all cursor-pointer outline-none", status === 'absent' ? "bg-red-500 border-red-500 text-white shadow-lg" : status === 'excused' ? "bg-amber-500 border-amber-500 text-white" : status === 'present' ? "bg-green-500 border-green-500 text-white shadow-md" : "bg-white border-slate-100 text-slate-200")}
      >
        {status === 'absent' ? <XCircle size={28} strokeWidth={2.5} /> : status === 'excused' ? "📝" : status === 'present' ? <CheckCircle2 size={28} strokeWidth={2.5} /> : <div className="w-4 h-4 rounded-full border-2 border-slate-100"></div>}
      </button>
    </div>
  );
}, (prev: any, next: any) => prev.student.id === next.student.id && prev.status === next.status);
...
export default function HalqaPage() {
  ...
  const attendanceRef = useRef<Record<string, any>>({});
  ...
  const [tempAbsenceThreshold, setTempAbsenceThreshold] = useState(3);
  ...
  // --- Helpers (memoized to prevent re-creation on every render) ---
  const isHoliday = useCallback((date: Date | string) => {
    const d = typeof date === 'string' ? parseISO(date) : date;
    return weekendDays.includes(d.getDay());
  }, [weekendDays]);

  const monthStr = useMemo(() => format(currentDate, "yyyy-MM"), [currentDate]);
  const todayStrLocal = useMemo(() => format(new Date(), "yyyy-MM-dd"), []);
  ...
  const studentAbsenceCounts = useMemo(() => {
    ...
    Object.entries(attendance).forEach(([key, status]) => {
      ...
      const sId = key.slice(0, -11);
      if (!dStr.startsWith(monthStr)) return;
      const d = parseISO(dStr);
      if (!weekendDays.includes(d.getDay())) counts[sId] = (counts[sId] || 0) + 1;
    });
    return counts;
  }, [attendance, monthStr, weekendDays, students]);

  const totalAbsenceCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    students.forEach(s => counts[s.id] = 0);
    Object.entries(attendance).forEach(([key, status]) => {
      if (status !== 'absent') return;
    ...
    return counts;
  }, [attendance, weekendDays, students]);

  const daysInfo = useMemo(() => {
    const start = startOfMonth(currentDate);
    const end = endOfMonth(currentDate);
    const dList = eachDayOfInterval({ start, end });
    ...
    return dList.map(d => ({
      ...
      isHol: isHoliday(d),
      dayName: format(d, "EEE", { locale: ar }).split(' ')[0],
      dayNameFull: format(d, "EEE", { locale: ar }),
      dayNum: format(d, "d"),
      dayNumMonth: format(d, "d/M"),
    }));
  }, [currentDate, isHoliday]);

  const workingDaysInfo = useMemo(() => daysInfo.filter(d => !d.isHol), [daysInfo]);

  // --- Fetch Data --- only re-runs when halqaId or month changes
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { if (halqaId) fetchData(); }, [halqaId, currentDate]);


  // Keep attendanceRef synced with the latest state
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`attendance_changes_${halqaId}`)
      .on(
        'postgres_changes',
        {
          ...
          schema: 'public',
          table: 'attendance',
          filter: `halqa_id=eq.${halqaId}`
        },
        async (payload) => {
          const newRecord = payload.new as any;
          if (newRecord && newRecord.student_id && newRecord.date) {
            const key = `${newRecord.student_id}-${newRecord.date}`;

            // 1. Update React UI instantly
            setAttendance(prev => {
              // Ignore if we already have the exact same status (prevents unnecessary re-renders when WE are the ones who updated it)
              if (prev[key] === newRecord.status) return prev;
              return { ...prev, [key]: newRecord.status };
            });

            // 2. Update Local Storage silently
            await db.attendance.put(newRecord).catch(() => { });
          }
        }
      )
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'students', filter: `halqa_id=eq.${halqaId}` },
        async (payload) => {
          if (payload.eventType === 'INSERT') {
            const newStudent = payload.new as any;
            setStudents(prev => {
              if (prev.find(s => s.id === newStudent.id)) return prev;
              return [...prev, newStudent].sort((a, b) => a.name.localeCompare(b.name, 'ar'));
            });
            await db.students.put(newStudent).catch(() => { });
          } else if (payload.eventType === 'UPDATE') {
            const updatedStudent = payload.new as any;
            setStudents(prev => prev.map(s => s.id === updatedStudent.id ? updatedStudent : s).sort((a, b) => a.name.localeCompare(b.name, 'ar')));
            await db.students.put(updatedStudent).catch(() => { });
          } else if (payload.eventType === 'DELETE') {
            const deletedStudent = payload.old as any;
            setStudents(prev => prev.filter(s => s.id !== deletedStudent.id));
            await db.students.delete(deletedStudent.id).catch(() => { });
          }
        }
      )
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'halqa_members', filter: `halqa_id=eq.${halqaId}` },
        () => {
          // If a teacher was removed, we just refetch teachers so it updates in settings modal
          supabase.from("invitations").select("*").eq("halqa_id", halqaId).then(({ data }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (cachedHalqa) {
        ...
        // Only block UI if we don't have basic halqa data
        setLoading(true);
      }
      ...
      if (user) {
        setCurrentUser(user);
        if (cachedHalqa && cachedHalqa.created_by === user.id) setIsAdmin(true);
      }
      ...
      const cachedAttendance = await db.attendance
        .where('halqa_id').equals(halqaId as string)
        .and(r => r.date >= monthStart && r.date <= monthEnd)
        .toArray();
      ...
      if (cachedHalqa) setLoading(false);

      // 2. Silently Revalidate from Supabase (Stale-While-Revalidate)
      if (isOnline) {
        const isStale = !cachedHalqa || !cachedHalqa.last_fetched || (Date.now() - cachedHalqa.last_fetched > 5 * 60 * 1000);

        if (isStale) {
          const [halqaRes, sRes] = await Promise.all([
            supabase.from("halqas").select("*").eq("id", halqaId).single(),
            supabase.from("students").select("*").eq("halqa_id", halqaId).order("name"),
          ]);
          if (halqaRes.data) {
            const halqa = halqaRes.data;
            setAbsenceThreshold(halqa.settings?.absence_threshold || 3);
            setWeekendDays(halqa.settings?.weekend_days || [5, 6]);
            await db.halqas.put({ id: halqa.id, name: halqa.name, settings: halqa.settings, created_by: halqa.created_by, last_fetched: Date.now() });

            if (user && halqa.created_by === user.id) setIsAdmin(true);
          }
          if (sRes.data) {
            setStudents(sRes.data);
            await db.students.bulkPut(sRes.data);
          }
        }
        ...
        if (aData) {
          ...
          if (JSON.stringify(freshMap) !== JSON.stringify(attMap)) {
        ...
        // Fetch teachers for settings
        const { data: invData } = await supabase.from("invitations").select("*").eq("halqa_id", halqaId);
        if (invData) setTeachers(invData);

        // Re-check admin status in case cache missed it
        const { data: hData } = await supabase.from("halqas").select("created_by").eq("id", halqaId).single();
        const amIAdmin = user && hData?.created_by === user.id;
        if (amIAdmin) setIsAdmin(true);

        // Implicit Kick System: If I am not the admin, check if my invitation still exists
        if (!amIAdmin && user && user.email) {
          const { data: myInvite } = await supabase.from("invitations").select("id").eq("halqa_id", halqaId).eq("email", user.email.toLowerCase()).single();
          if (!myInvite) {
            await supabase.from("halqa_members").delete().eq("halqa_id", halqaId).eq("user_id", user.id);
  ...
  const handleToggle = useCallback(async (studentId: string, date: Date, forceType?: 'absent' | 'excused') => {
    console.log(`[handleToggle] INIT! student: ${studentId}, date:`, date);
    if (isHoliday(date)) {
      console.warn(`[handleToggle] BLOCKED! Date is a holiday. day of week = ${date.getDay()}`);
      return;
    }
    const dateStr = format(date, "yyyy-MM-dd");
    const key = `${studentId}-${dateStr}`;
    const currentStatus = attendanceRef.current[key]; // Always use the freshest state

    console.log(`[handleToggle] key: ${key}, currentStatus: ${currentStatus}`);
    ...
    // Optimistically update ref so subsequent fast clicks are aware immediately
    attendanceRef.current[key] = newStatus;
  ...
  // --- Manual Refresh: fetch students & attendance on demand ---
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      const [sRes, aRes] = await Promise.all([
        supabase.from("students").select("*").eq("halqa_id", halqaId).order("name"),
        supabase.from("attendance").select("*").eq("halqa_id", halqaId).gte("date", monthStart).lte("date", monthEnd),
      ]);

      if (sRes.data) {
        setStudents(sRes.data);
        await db.students.bulkPut(sRes.data).catch(() => {});
      }

      if (aRes.data) {
        const freshMap: Record<string, any> = {};
        aRes.data.forEach(r => freshMap[`${r.student_id}-${r.date}`] = r.status);
        setAttendance(freshMap);
        await db.attendance
          .where('halqa_id').equals(halqaId as string)
          .and(r => r.date >= monthStart && r.date <= monthEnd)
        ...
        if (aRes.data.length > 0) {
          await db.attendance.bulkPut(aRes.data.map(r => ({ student_id: r.student_id, halqa_id: r.halqa_id, date: r.date, status: r.status })));
        }
      }
    } catch (err) {
  ...
  }, [isRefreshing, isOnline, halqaId, currentDate]);

  const saveSettings = async () => {
    setIsSettingsSaving(true);
    const newSettings = { absence_threshold: Number(tempAbsenceThreshold), weekend_days: tempWeekendDays };

    // 1. Optimistic UI update
    setWeekendDays(tempWeekendDays);
    setAbsenceThreshold(tempAbsenceThreshold);
    db.halqas.update(halqaId as string, { settings: newSettings }).catch(console.error);

    // 2. Background sync
    await supabase.from("halqas").update({ settings: newSettings }).eq("id", halqaId);
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    if (!error && data) {
      ...
      setNewStudent({ name: "", parent_phone: "", notes: "", gender: 'male' });
    } else if (error) {
  ...
  const handleBulkAdd = async () => {
    try {
      ...
      if (data) { await db.students.bulkPut(data); setStudents(prev => [...prev, ...data].sort((a, b) => a.name.localeCompare(b.name, 'ar'))); setIsBulkAddOpen(false); setBulkJson(""); }
  ...
  const handleInvite = async () => {
    ...
    if (!email.trim()) { setInviteError("الرجاء إدخال البريد الإلكتروني"); return; }
    setInviteLoading(true);
    setInviteError("");
    try {
      // Use cached session (no network round-trip) instead of getUser()
      const { data: { session } } = await supabase.auth.getSession();
      if (!session?.user) { setInviteError("يجب تسجيل الدخول أولاً"); setInviteLoading(false); return; }
      const { error } = await supabase
        .from("invitations")
        .insert([{ halqa_id: halqaId, email: email.toLowerCase().trim(), role: 'editor', invited_by: session.user.id }]);
      if (error) {
        if (error.code === '23505') throw new Error("هذا المعلم مدعو أو موجود بالفعل في هذه الحلقة");
        throw error;
      }
      ...
      setInviteError(e?.message || "حدث خطأ، حاول مجدداً");
    } finally {
      setInviteLoading(false);
    }
  };

  const handleLeaveHalqa = async () => {
    if (!confirm("هل أنت متأكد من رغبتك في مغادرة هذه الحلقة؟ لن تتمكن من الدخول إليها مرة أخرى.")) return;
    try {
      if (!currentUser) return;
      await supabase.from("halqa_members").delete().eq("halqa_id", halqaId).eq("user_id", currentUser.id);
      await supabase.from("invitations").delete().eq("halqa_id", halqaId).eq("email", currentUser.email);
      router.push("/dashboard");
    } catch (e: any) { alert("حدث خطأ: " + e.message); }
  };

  const handleDeleteHalqa = async () => {
    if (!confirm("هل أنت متأكد من رغبتك في حذف هذه الحلقة بالكامل؟ سيتم مسح كل الطلاب والبيانات نهائياً!")) return;
    try {
      const { error } = await supabase.from("halqas").delete().eq("id", halqaId);
      if (error) throw error;
      await db.halqas.delete(halqaId as string);
      router.push("/dashboard");
    } catch (e: any) { alert("حدث خطأ في الحذف: " + (e.message || "تأكد من صلاحيات قاعدة البيانات")); }
  };

  const handleRemoveTeacher = async (email: string) => {
    if (!confirm(`هل أنت متأكد من إزالة المعلم ${email} من الحلقة؟`)) return;
    try {
      await supabase.from("invitations").delete().eq("halqa_id", halqaId).eq("email", email);
  ...
  }), [students, searchQuery, genderFilter, attendanceFilter, attendance, todayStrLocal]);

  const studentsWithWarnings = useMemo(() => students.filter(s => (studentAbsenceCounts[s.id] || 0) >= absenceThreshold), [students, studentAbsenceCounts, absenceThreshold]);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-24 md:pb-10 font-sans text-slate-900" dir="rtl">
      ...
      <div className="max-w-[1600px] mx-auto p-4 md:p-8 space-y-6">
        ...
        <header className="bg-white p-6 md:p-8 rounded-[32px] md:rounded-[40px] shadow-sm border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={() => { setTempWeekendDays(weekendDays); setTempAbsenceThreshold(absenceThreshold); setIsSettingsOpen(true); }} className="p-3 bg-slate-50 text-slate-600 rounded-xl border border-slate-100 shadow-sm"><Settings size={20} /></button>
            <button
        ...
        <div className="md:hidden">
              ...
              {isFastMode ? (
                <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm divide-y divide-slate-50">
                  ...
                  {filteredStudents.map(s => {
                    const status = attendance[`${s.id}-${todayStrLocal}`];
                    return (
                      <StudentRowMobileFast
                        key={s.id}
                        student={s}
                        todayStrLocal={todayStrLocal}
                        status={status}
                ...
                filteredStudents.map(s => {
                  const monthAbs = studentAbsenceCounts[s.id] || 0;
                  const totalAbs = totalAbsenceCounts[s.id] || 0;
                  return (
                    <StudentCardMobile
                      key={s.id}
                      student={s}
          ...
          {activeTab === 'stats' && (
            <div className="space-y-6">
              ...
              {/* Best / Worst Attendance List */}
              {(() => {
                const sortedByAbsence = [...students].sort((a, b) => (totalAbsenceCounts[a.id] || 0) - (totalAbsenceCounts[b.id] || 0));
                const bestStudents = sortedByAbsence.slice(0, 5);
                const worstStudents = sortedByAbsence.slice().reverse().slice(0, 5).filter(s => (totalAbsenceCounts[s.id] || 0) > 0);

                return (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-4">
                    <div className="bg-white rounded-[32px] p-6 border border-slate-100 shadow-sm">
                      <h3 className="font-black text-base flex items-center gap-2 mb-6 text-green-600"><CheckCircle2 size={20} /> الأفضل حضوراً</h3>
                      <div className="space-y-3">
                        {bestStudents.map((s, idx) => (
                          <div key={s.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                            <div className="w-8 h-8 rounded-full bg-white text-slate-400 font-black text-[10px] flex items-center justify-center border border-slate-200 flex-shrink-0">{idx + 1}</div>
                            <span className="font-black text-sm flex-1 truncate">{s.name}</span>
                            <div className="bg-green-100 text-green-700 px-3 py-1 rounded-xl text-[10px] font-black">{totalAbsenceCounts[s.id] ? `${totalAbsenceCounts[s.id]} غياب` : "مثالي 🌟"}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {worstStudents.length > 0 && (
                      <div className="bg-white rounded-[32px] p-6 border border-slate-100 shadow-sm">
                        <h3 className="font-black text-base flex items-center gap-2 mb-6 text-red-500"><AlertTriangle size={20} /> الأكثر غياباً</h3>
                        <div className="space-y-3">
                          {worstStudents.map((s, idx) => (
                            <div key={s.id} className="flex items-center gap-3 p-3 bg-red-50/50 rounded-2xl border border-red-100">
                              <div className="w-8 h-8 rounded-full bg-white text-red-400 font-black text-[10px] flex items-center justify-center border border-red-100 flex-shrink-0">{idx + 1}</div>
                              <span className="font-black text-sm flex-1 truncate">{s.name}</span>
                              <div className="bg-red-500 text-white px-3 py-1 rounded-xl text-[10px] font-black">{totalAbsenceCounts[s.id]} غياب</div>
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        <div className="hidden md:grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-white rounded-[32px] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden"><div className="p-5 border-b flex items-center justify-between bg-slate-50/50"><div className="flex items-center gap-4"><button onClick={() => setCurrentDate(d => { const n = new Date(d); n.setMonth(n.getMonth() - 1); n.setDate(1); return n; })} className="p-2 bg-white rounded-lg border hover:bg-slate-100 transition-all"><ChevronRight size={20} /></button><span className="font-black text-slate-800 text-lg">{format(currentDate, "MMMM yyyy", { locale: ar })}</span><button onClick={() => setCurrentDate(d => { const n = new Date(d); n.setMonth(n.getMonth() + 1); n.setDate(1); return n; })} className="p-2 bg-white rounded-lg border hover:bg-slate-100 transition-all"><ChevronLeft size={20} /></button></div><button onClick={scrollToToday} className="bg-blue-50 text-blue-700 px-4 py-2 rounded-xl font-black text-xs border border-blue-100 shadow-sm transition-all active:scale-95">الذهاب لليوم</button></div><div className="overflow-x-auto relative" ref={scrollContainerRef}><table className="w-full border-collapse"><thead><tr className="bg-slate-50/30 font-black text-[10px] text-slate-400 uppercase tracking-widest"><th className="sticky right-0 z-30 bg-white p-6 text-right border-l border-b min-w-[240px]">اسم الطالب</th><th className="p-6 text-center border-l border-b min-w-[100px]">الغياب</th>{daysInfo.map(d => (<th key={d.dateStr} ref={d.isToday ? todayRef : null} className={cn("p-4 text-center border-l border-b min-w-[65px] relative", d.isToday ? "bg-blue-600 text-white shadow-lg" : "", d.isHol && "bg-orange-50/50 text-orange-400")}><div className="text-[10px] font-black">{d.dayNameFull}</div><div className="text-xl font-black">{d.dayNum}</div></th>))}</tr></thead><tbody>{filteredStudents.map(s => { const mAbs = studentAbsenceCounts[s.id] || 0; return (<StudentRowDesktop key={s.id} student={s} daysInfo={daysInfo} mAbs={mAbs} absenceThreshold={absenceThreshold} attendanceMap={attendance} onToggle={handleToggle} onSelect={setSelectedStudent} />); })}</tbody></table></div></div>
          </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-100 p-4 pb-6 flex justify-around items-center z-[150] shadow-lg">
        <button onClick={() => router.push("/dashboard")} className="flex flex-col items-center gap-1 text-slate-400"><LayoutGrid size={24} /><span className="text-[10px] font-black">حلقات</span></button>
        ...
        <button
          ...
          className={cn(
            ...
            isRefreshing ? "text-emerald-500" : !isOnline ? "text-slate-200" : "text-emerald-500"
          )}
        >
          <div className={cn("p-2 rounded-2xl transition-all", isRefreshing && "bg-emerald-100")}>
            <RefreshCw size={24} className={cn(isRefreshing && "animate-spin")} />
          </div>
          <span className="text-[10px] font-black">{isRefreshing ? "..." : "تحديث"}</span>
        </button>
      ...
      {/* --- ALL MODALS --- */}
      {isInviteModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center bg-slate-900/40 backdrop-blur-sm p-0 md:p-4" onClick={(e) => { if (e.target === e.currentTarget) { setIsInviteModalOpen(false); setInviteError(""); } }}>
          <div className="bg-white w-full max-w-md rounded-t-[40px] md:rounded-[40px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-20 duration-300">
            <div className="bg-blue-600 p-8 text-white text-center relative">
              <button onClick={() => { setIsInviteModalOpen(false); setInviteError(""); }} className="absolute top-6 left-6 w-10 h-10 bg-white/20 hover:bg-white/30 rounded-xl flex items-center justify-center text-white transition-all"><XCircle size={20} /></button>
              <div className="w-14 h-14 bg-white/20 rounded-2xl flex items-center justify-center mx-auto mb-3"><Mail size={28} /></div>
              <h3 className="text-2xl font-black text-white">دعوة معلم للحلقة</h3>
              <p className="text-blue-100 text-xs font-bold mt-1 opacity-80">سيتمكن المعلم من الوصول لهذه الحلقة</p>
            </div>
            <div className="p-8 space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase px-1">البريد الإلكتروني للمعلم</label>
                <input
                  ref={inviteEmailRef}
                  type="email"
                  defaultValue=""
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 font-bold outline-none text-slate-800 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-300 transition-all"
                  dir="ltr"
                ...
                {inviteError && (
                  <div className="flex items-center gap-2 text-red-600 bg-red-50 p-3 rounded-xl border border-red-100">
                    ...
                    <span className="text-xs font-bold">{inviteError}</span>
                  </div>
                )}
              </div>
              <div className="flex gap-4 pt-2">
                <button
                  ...
                  disabled={inviteLoading}
                  className="flex-1 bg-blue-600 text-white font-black py-4 rounded-2xl shadow-lg shadow-blue-200 flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed transition-all active:scale-95"
                >
                  {inviteLoading ? <><Loader2 size={18} className="animate-spin" /> جاري الإرسال...</> : <><Mail size={18} /> إرسال الدعوة</>}
                </button>
                <button onClick={() => { setIsInviteModalOpen(false); setInviteError(""); }} disabled={inviteLoading} className="flex-1 bg-slate-100 text-slate-500 font-black py-4 rounded-2xl disabled:opacity-60 transition-all">إلغاء</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center bg-slate-900/40 backdrop-blur-sm p-0 md:p-4">
          <div className="bg-white w-full max-w-lg rounded-t-[40px] md:rounded-[40px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-20 duration-300">
            {isEditing ? (
              ...
              <div className="pb-8">
                {/* Header Cover */}
                <div className={cn("h-40 p-6 relative flex items-end justify-between", selectedStudent.gender === 'female' ? "bg-gradient-to-br from-pink-500 via-pink-600 to-rose-600" : "bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-600")}>
                  {/* Decorative pattern */}
                  <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-white to-transparent"></div>

                  <button onClick={() => setSelectedStudent(null)} className="absolute top-4 right-4 w-10 h-10 bg-white/20 hover:bg-white/30 backdrop-blur-md rounded-2xl flex items-center justify-center text-white transition-all z-10"><X size={20} /></button>

                  <button onClick={() => { setIsEditing(true); setEditStudent(selectedStudent); }} className="absolute top-4 left-4 px-4 h-10 bg-white/20 hover:bg-white/30 backdrop-blur-md rounded-2xl flex items-center justify-center text-white font-bold text-[10px] transition-all z-10 flex gap-2"><Edit2 size={12} /> تعديل البيانات</button>
                </div>

                {/* Profile Info */}
                <div className="px-8 relative z-10">
                  <div className="flex flex-col gap-4 mb-8">
                    <div className={cn("-mt-12 w-24 h-24 rounded-[32px] shadow-xl flex items-center justify-center text-4xl font-black border-4 border-white flex-shrink-0", selectedStudent.gender === 'female' ? "bg-pink-100 text-pink-600" : "bg-blue-100 text-blue-600")}>
                      {selectedStudent.name.charAt(0)}
                    </div>
                    <div className="pb-2">
                      <h3 className="text-2xl font-black text-slate-800 leading-tight">{selectedStudent.name}</h3>
                      {selectedStudent.parent_phone && (
                        <a href={`tel:${selectedStudent.parent_phone}`} className="inline-flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-xl text-[10px] font-black text-slate-500 mt-2 transition-all shadow-sm" dir="ltr"><Phone size={12} /> {selectedStudent.parent_phone}</a>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-6">
                    <div className="bg-red-50 p-4 rounded-3xl border border-red-100/50 flex flex-col items-center justify-center text-center shadow-sm">
                      <div className="w-10 h-10 rounded-full bg-red-100 text-red-500 flex items-center justify-center mb-2"><XCircle size={20} /></div>
                      <p className="text-2xl font-black text-red-600 leading-none">{totalAbsenceCounts[selectedStudent.id] || 0}</p>
                      <p className="text-red-400 text-[9px] font-black uppercase mt-2 tracking-widest">غياب كلي</p>
                    </div>
                    <div className="bg-amber-50 p-4 rounded-3xl border border-amber-100/50 flex flex-col items-center justify-center text-center shadow-sm">
                      <div className="w-10 h-10 rounded-full bg-amber-100 text-amber-500 flex items-center justify-center mb-2"><AlertTriangle size={20} /></div>
                      <p className="text-2xl font-black text-amber-600 leading-none">{Object.keys(attendance).filter(k => k.startsWith(`${selectedStudent.id}-`) && attendance[k] === 'excused').length}</p>
                      <p className="text-amber-500 text-[9px] font-black uppercase mt-2 tracking-widest">استئذان</p>
                    </div>
                    <div className="bg-slate-900 p-4 rounded-3xl flex flex-col items-center justify-center text-center shadow-xl shadow-slate-900/10 border border-slate-800">
                      <div className="w-10 h-10 rounded-full bg-white/10 text-white flex items-center justify-center mb-2"><Calendar size={20} /></div>
                      <p className="text-2xl font-black text-white leading-none">{studentAbsenceCounts[selectedStudent.id] || 0}</p>
                      <p className="text-slate-400 text-[9px] font-black uppercase mt-2 tracking-widest">غياب الشهر</p>
                    </div>
                  </div>

                  {/* Commitment Progress */}
                  <div className="bg-gradient-to-r from-green-500 to-emerald-500 p-6 rounded-3xl text-white relative overflow-hidden mb-6 shadow-lg shadow-green-500/20">
                    <Target size={120} className="absolute -left-6 -top-6 opacity-10 rotate-12" />
                    <div className="relative z-10">
                      <div className="flex justify-between items-end mb-4">
                        <div>
                          <p className="text-green-100 text-[10px] font-black uppercase mb-1 tracking-widest">مؤشر الالتزام</p>
                          <p className="text-4xl font-black">{Math.max(0, 100 - ((totalAbsenceCounts[selectedStudent.id] || 0) * 3))}%</p>
                        </div>
                        <div className="w-12 h-12 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center"><CheckCircle2 size={24} className="text-white" /></div>
                      </div>
                      <div className="h-2 bg-white/20 rounded-full overflow-hidden">
                        <div className="h-full bg-white rounded-full transition-all duration-1000 ease-out" style={{ width: `${Math.max(0, 100 - ((totalAbsenceCounts[selectedStudent.id] || 0) * 3))}%` }}></div>
                      </div>
                    </div>
                  </div>

                  {/* Actions */}
                  <button onClick={() => { if (confirm("هل أنت متأكد من حذف هذا الطالب نهائياً من الحلقة؟")) { const id = selectedStudent.id; supabase.from("students").delete().eq("id", id).then(({ error }) => { if (!error) { db.students.delete(id).catch(() => { }); setStudents(prev => prev.filter(s => s.id !== id)); setSelectedStudent(null); } else { alert("خطأ في الحذف: " + error.message); } }); } }} className="w-full p-4 bg-red-50 text-red-600 rounded-2xl hover:bg-red-100 transition-all border border-red-100 font-black flex items-center justify-center gap-2 text-sm"><Trash2 size={18} /> حذف الطالب من الحلقة</button>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center bg-slate-900/40 backdrop-blur-sm p-0 md:p-4">
          <div className="bg-white w-full max-w-md rounded-t-[40px] md:rounded-[40px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-20 duration-300">
            ...
            <div className="p-8 space-y-8">
              {/* Teachers Settings (For Admin or Leaving) */}
              <div className="space-y-4">
                <label className="text-[10px] font-black text-slate-400 uppercase">إدارة المعلمين</label>

                {isAdmin ? (
                  <>
                    <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 space-y-3 max-h-[150px] overflow-y-auto">
                      {teachers.map(t => (
                        <div key={t.id} className="flex justify-between items-center text-sm font-bold text-slate-800 bg-white p-3 rounded-2xl shadow-sm">
                          <span className="truncate flex-1" dir="ltr">{t.email} <span className="text-[10px] text-slate-400">({t.status === 'accepted' ? 'منضم' : 'معلق'})</span></span>
                          {t.email !== currentUser?.email && (
                            <button onClick={() => handleRemoveTeacher(t.email)} className="text-red-500 bg-red-50 p-2 rounded-xl hover:bg-red-100 transition-all mr-2"><Trash2 size={14} /></button>
                          )}
                        </div>
                      ))}
                      {teachers.length === 0 && <p className="text-xs text-slate-400 text-center font-bold">لا يوجد معلمون آخرون</p>}
                    </div>
                    <button onClick={handleDeleteHalqa} className="w-full mt-2 p-3 bg-red-50 text-red-600 rounded-2xl font-black text-sm border border-red-100 flex items-center justify-center gap-2 hover:bg-red-100 transition-all"><AlertTriangle size={16} /> حذف الحلقة نهائياً</button>
                  </>
                ) : (
                  <button onClick={handleLeaveHalqa} className="w-full mt-2 p-3 bg-red-50 text-red-600 rounded-2xl font-black text-sm border border-red-100 flex items-center justify-center gap-2 hover:bg-red-100 transition-all"><XCircle size={16} /> مغادرة الحلقة</button>
                )}
              </div>

              {/* Halqa Settings */}
              <div className="space-y-4"><label className="text-[10px] font-black text-slate-400 uppercase">أيام الإجازة</label><div className="flex flex-wrap gap-2">{["أحد", "اثنين", "ثلاثاء", "أربعاء", "خميس", "جمعة", "سبت"].map((day, i) => (<button key={day} onClick={() => setTempWeekendDays(prev => prev.includes(i) ? prev.filter(d => d !== i) : [...prev, i])} className={cn("flex-1 min-w-[60px] py-3 rounded-xl font-bold text-[10px] border transition-all", tempWeekendDays.includes(i) ? "bg-blue-600 text-white border-blue-600 shadow-sm" : "bg-white text-slate-400 border-slate-200")}>{day}</button>))}</div></div>
              <div className="space-y-4"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">حد إنذار الغياب ({tempAbsenceThreshold})</label><input type="range" min="1" max="15" value={tempAbsenceThreshold} onChange={e => setTempAbsenceThreshold(parseInt(e.target.value))} className="w-full accent-blue-600 h-2 bg-slate-100 rounded-lg appearance-none cursor-pointer" /></div>

              <button onClick={saveSettings} disabled={isSettingsSaving} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-2xl shadow-lg shadow-blue-200 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
                {isSettingsSaving ? <><Loader2 size={20} className="animate-spin" /> جاري الحفظ...</> : "حفظ التعديلات"}
              </button>
            </div>
          </div>
        </div>
